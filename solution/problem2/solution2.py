...
from lib.utils import calcTime,passengerFlow
import random
...
def calcTotalFlow(map):
    ...
    for tkey, tval in Tickets.items():
        ...
        # 当到达和出发航班都找到了登机口的时候换乘成功
        if arrive_flight != None and leave_flight != None:
            route,cnt = passengerFlow(arrive_flight,leave_flight)
    ...
    return flow,int(failed),int(total_people)
...
if __name__ == '__main__':
    ...
    for i in range(N):
        ...
        flow,n_fail,n_all = calcTotalFlow(Gmap)
        # 航班分配最多
        if score > optscore:
            optscore = score
            opt_gmap = Gmap
            ...
            with open('result.txt','w') as f:
                ...
                f.close()
        # 流程最短
        elif score == optscore and sum(flow) < sum(opt_flow):
            optscore = score
            opt_gmap = Gmap
            opt_n_fail = n_fail
            opt_n_all = n_all
        ...
        print('Iteration %d: score:%d, flow:%d, optscore:%d,opt_gate:%d, num of flow: %d'%(i+1,score,sum(flow),optscore,len(opt_gmap),sum(opt_flow)))
